...
import { getPassengerActiveRide, expireStaleRides } from '../../services/rideService'
import {
  getCurrentPosition,
  reverseGeocode,
  geocodeAddress,
...
export default function PassengerDashboard() {
  ...
  useEffect(() => {
    let mounted = true
    // Antes de tudo, fecha pedidos antigos que ficaram esperando sem ninguém aceitar
    expireStaleRides().then(() => getPassengerActiveRide(user.id)).then((ride) => {
      if (!mounted) return
      if (ride) {
        watchRide(ride.id)
